...
export default function CenturyPage({ params }) {
  ...
  React.useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await fetch(`/api/books/century/${params.reference}`)
  ...
  const centuryName = books.length > 0 ? books[0].century_name : params.reference
          ...
          <Container>
              ...
              <Col>
                <Breadcrumb>
                  ...
                  <BreadcrumbItem active>
                    <Link href={`/legacy-library/books/century/${params.reference}`}>{deEntitize(centuryName || params.reference)}</Link>
                  </BreadcrumbItem>
                </Breadcrumb>
              </Col>
              <Col xs={2}>
            ...
            <Row className="justify-content-center">
              <Col lg="11" className="mb-4 mb-lg-5">
                <Box>
                  <Title variant="hero">Books from the {deEntitize(centuryName || params.reference)}</Title>
                </Box>
              </Col>
            </Row>
            <Row>
