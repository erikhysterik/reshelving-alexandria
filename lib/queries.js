...
// Get author's books (including as illustrator)
export async function getAuthorBooks(authorId) {
  const sql = `
    SELECT book.cs_rid, book.title, book.publication_date, book.reference,
           GROUP_CONCAT(DISTINCT author_book_l.cs_type ORDER BY author_book_l.cs_type) as roles
    FROM book
    INNER JOIN author_book_l ON book.cs_rid = author_book_l.book_id
    WHERE author_book_l.author_id = ? AND (author_book_l.cs_type = 'basic' OR author_book_l.cs_type = 'illustrator')
    AND book.status <> 'draft' AND book.status_notes NOT LIKE '%hold%'
    GROUP BY book.cs_rid, book.title, book.publication_date, book.reference
  ...
  return results.map(book => ({
    ...book,
    // For backward compatibility, set cs_type to the first role found
    cs_type: book.roles.split(',')[0],
    // Add all roles for UI display
    all_roles: book.roles.split(',')
  }))
}