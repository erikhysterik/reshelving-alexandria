...
function Century(props) {
  const { centurydetails } = props.data
  ...
  const [bookList, setBookList] = React.useState([]);
    ...
    const afterPageClicked = (page_number) => {
      ...
      let endex = Math.min((page_number * 50), centurydetails.centurybooks.length);

       setBookList(centurydetails.centurybooks.slice(((page_number-1)*50), endex));
    };
    ...
    <>
      ...
      <Row className="d-flex align-items-center">
        ...
        <Breadcrumb>
          ...
          <BreadcrumbItem linkAs={Link} linkProps={{to: '/legacy-library/century'}} title="By Century" active={true} >By Century</BreadcrumbItem>
          </Breadcrumb>
         </Col>
         <Col xs={2}>
      ...
      <Row className="justify-content-center">
              ...
              <Box>
                    <Title variant="hero">{centurydetails.name}</Title>
                  </Box>
              </Col>
            </Row>
            <Row>
              ...
              <Card.Body>
                <Card.Text>{centurydetails.description}</Card.Text>
              </Card.Body>
          </Card>
                </Col>
            </Row>
              ...
              <Col lg="12" className="mb-4 mb-lg-5">
                <Card>
                  <Card.Body>
                    ...
                    <Card.Body>
                    <CustomPagination
      totPages={centurydetails.centurybooks.length % 50 ? centurydetails.centurybooks.length / 50 + 1 : centurydetails.centurybooks.length / 50 }
...
query ($id: String!) {
  centurydetails: mysqlCentury(id: { eq: $id }) {
      centurybooks {
        first
        last
        publication_date
