...
function MajorTimeperiod(props) {
  const { majortimeperioddetails } = props.data
  ...
  const [bookList, setBookList] = React.useState([]);
    ...
    const afterPageClicked = (page_number) => {
      ...
      let endex = Math.min((page_number * 50), majortimeperioddetails.majortimeperiodbooks.length);

       setBookList(majortimeperioddetails.majortimeperiodbooks.slice(((page_number-1)*50), endex));
    };
    ...
    <>
      ...
      <Row className="d-flex align-items-center">
        ...
        <Breadcrumb>
          ...
          <BreadcrumbItem linkAs={Link} linkProps={{to: '/legacy-library/books/timeperiod/major'}} title="By Major Timeperiod" active={true} >By Major Timeperiod</BreadcrumbItem>
          </Breadcrumb>
         </Col>
         <Col xs={2}>
      ...
      <Row className="justify-content-center">
              ...
              <Box>
                    <Title variant="hero">{majortimeperioddetails.name}</Title>
                  </Box>
              </Col>
            </Row>
            <Row>
              ...
              <Card.Body>
                <Card.Text>{majortimeperioddetails.period}</Card.Text>
              </Card.Body>
          </Card>
                </Col>
            </Row>
              ...
              <Col lg="12" className="mb-4 mb-lg-5">
                <Card>
                  <Card.Body>
                    ...
                    <Card.Body>
                    <CustomPagination
      totPages={majortimeperioddetails.majortimeperiodbooks.length % 50 ? majortimeperioddetails.majortimeperiodbooks.length / 50 + 1 : majortimeperioddetails.majortimeperiodbooks.length / 50 }
...
export default MajorTimeperiod

export const query = graphql`
query ($id: String!) {
    majortimeperioddetails: mysqlMajortimeperiod(id: { eq: $id }) {
      majortimeperiodbooks {
        first
        last
        publication_date
        reference
